...
async function listCommands() {
  ...
  const _commands = await Promise.all(commandsOrder.map((cmd) => index.getCommand(cmd)));
  let maxLength = 0;
  const commandsHelp = [];
  ...
  const _cmds = commandsHelp.map(([cmd, description]) => {
    ...
    return index.foldLines(chalk__default["default"].green(cmd) + i + description, index.startSpaces + maxLength + index.optionSpaces * 2, index.startSpaces + index.optionSpaces);
  }).join("\n");
