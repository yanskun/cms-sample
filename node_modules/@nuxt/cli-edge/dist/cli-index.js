/*!
 * @nuxt/cli-edge v2.16.0-27260655.293dc11a (c) 2016-2021
...
const importModule = (id) => {
  try {
    return Promise.resolve(utilsEdge.requireModule(id));
  } catch (err) {
...
const builder = () => importModule("@nuxt/builder-edge");
const webpack = () => importModule("@nuxt/webpack-edge");
const generator = () => importModule("@nuxt/generator-edge");
const core = () => importModule("@nuxt/core-edge");
const server$1 = () => importModule("@nuxt/server-edge");
...
function colorize(text) {
  return text.replace(/\[[^ ]+]/g, (m) => chalk__default["default"].grey(m)).replace(/<[^ ]+>/g, (m) => chalk__default["default"].green(m)).replace(/ (-[-\w,]+)/g, (m) => chalk__default["default"].bold(m)).replace(/`([^`]+)`/g, (_, m) => chalk__default["default"].bold.cyan(m));
}
...
const eventsMapping = {
  ...
  change: { icon: env__default["default"].windows ? "\xBB" : "\u21BB", color: "blue", action: "Updated" },
...
const common = {
  ...
  "config-file": {
    alias: "c",
    type: "string",
    default: configEdge.defaultNuxtConfigFile,
...
var name = "@nuxt/cli-edge";
var version = "2.16.0-27260655.293dc11a";
...
async function loadNuxtConfig(argv, configContext) {
  ...
  const options = await configEdge.loadNuxtConfig({
    rootDir,
    configFile,
    configContext,
    envConfig: {
  ...
  }, options.server || {}, configEdge.getDefaultNuxtConfig().server);
  return options;
}
...
`);
  ...
  _getHelp() {
    ...
    const _opts = options.map(([option, description2]) => {
      ...
      return foldLines(option + i + description2, startSpaces + maxOptionLength + optionSpaces * 2, startSpaces + optionSpaces);
    }).join("\n");
...
function getInstalledVersion(name) {
  try {
    return utilsEdge.getPKG(name).version;
  } catch {
  }
}
...
function isNuxtDir(rootDir) {
  if (fs__default["default"].existsSync(path__default["default"].join(rootDir, "nuxt.config.js")) || fs__default["default"].existsSync(path__default["default"].join(rootDir, "pages")) || fs__default["default"].existsSync(path__default["default"].join(rootDir, "nuxt.config.ts"))) {
    return true;
  }
  return false;
}
...
async function run(_argv, hooks = {}) {
  ...
  if (fs.existsSync(dupPkgJSON) && utilsEdge.requireModule(dupPkgJSON).name !== "@nuxt/" + dupPkg) {
