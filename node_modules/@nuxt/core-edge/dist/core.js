/*!
 * @nuxt/core-edge v2.16.0-27260655.293dc11a (c) 2016-2021
...
const serverEdge = require('@nuxt/server-edge');
...
class ModuleContainer {
  ...
  async ready() {
    ...
    if (this.options.buildModules && !this.options._start) {
      await utilsEdge.sequence(this.options.buildModules, this.addModule);
    }
    await utilsEdge.sequence(this.options.modules, this.addModule);
    await utilsEdge.sequence(this.options._modules, this.addModule);
  ...
  extendBuild(fn) {
    this.options.build.extend = utilsEdge.chainFn(this.options.build.extend, fn);
  }
  extendRoutes(fn) {
    this.options.router.extendRoutes = utilsEdge.chainFn(this.options.router.extendRoutes, fn);
  }
...
class Resolver {
  constructor(nuxt) {
    ...
    this._createRequire = this.options.createRequire || utilsEdge.createRequire;
  ...
  resolveAlias(path$1) {
    if (utilsEdge.startsWithRootAlias(path$1)) {
      return path.join(this.options.rootDir, path$1.substr(2));
    }
    if (utilsEdge.startsWithSrcAlias(path$1)) {
  ...
  requireModule(path, { alias, isAlias = alias, intropDefault, interopDefault = intropDefault, paths } = {}) {
    ...
    const isExternal = utilsEdge.isExternalDependency(resolvedPath);
    if (this.options.dev && !isExternal) {
      utilsEdge.clearRequireCache(resolvedPath);
    }
    try {
...
class Nuxt extends Hookable__default["default"] {
  constructor(options = {}) {
    ...
    this.options = configEdge.getNuxtConfig(options);
    ...
    utilsEdge.defineAlias(this, this.resolver, ["resolveAlias", "resolvePath"]);
    this.showReady = () => {
  ...
  _initServer() {
    ...
    this.server = new serverEdge.Server(this);
    this.renderer = this.server;
    this.render = this.server.app;
    utilsEdge.defineAlias(this, this.server, ["renderRoute", "renderAndGetWindow", "listen"]);
  }
...
async function loadNuxt(loadOptions) {
  ...
  const config = await configEdge.loadNuxtConfig(loadOptions);
...
Object.defineProperty(exports, 'loadNuxtConfig', {
  enumerable: true,
  get: function () { return configEdge.loadNuxtConfig; }
});
