/*!
 * @nuxt/generator-edge v2.16.0-27260655.293dc11a (c) 2016-2021
...
class Generator {
  constructor(nuxt, builder) {
    ...
    this.isFullStatic = utilsEdge.isFullStatic(this.options);
    ...
    this.distNuxtPath = path__default["default"].join(this.distPath, utilsEdge.isUrl(this.options.build.publicPath) ? "" : this.options.build.publicPath);
    if (this.isFullStatic) {
      ...
      this.staticAssetsBase = utilsEdge.urlJoin(this.options.app.cdnURL, this.options.generate.staticAssets.versionBase);
      if (manifest) {
  ...
  async initiate({ build = true, init = true } = {}) {
    ...
    if (build) {
      if (!this.builder) {
        throw new Error(`Could not generate. Make sure a Builder instance is passed to the constructor of \`Generator\` class or \`getGenerator\` function or disable the build step: \`generate({ build: false })\``);
      }
      ...
      if (!hasBuilt) {
        throw new Error(`No build files found in ${this.srcBuiltPath}.
Please run \`nuxt build\``);
  ...
  async initRoutes(...args) {
    ...
    if (this.options.router.mode !== "hash") {
      try {
        generateRoutes = await utilsEdge.promisifyRoute(this.options.generate.routes || [], ...args);
      } catch (e) {
    ...
    if (this.options.router.mode === "hash") {
      ...
      try {
        routes = utilsEdge.flatRoutes(this.getAppRoutes());
      } catch (err) {
        routes = ["/"];
      }
    }
  ...
  getBuildConfig() {
    try {
      return utilsEdge.requireModule(path__default["default"].join(this.options.buildDir, "nuxt/config.json"));
    } catch (err) {
      return null;
    }
  }
  getAppRoutes() {
    return utilsEdge.requireModule(path__default["default"].join(this.options.buildDir, "routes.json"));
  }
  async generateRoutes(routes) {
    ...
    while (this.routes.length) {
      let n = 0;
      await Promise.all(this.routes.splice(0, this.options.generate.concurrency).map(async ({ route, payload }) => {
        await utilsEdge.waitFor(n++ * this.options.generate.interval);
...
`);
      if (isHandled) {
        line += chalk__default["default"].grey(JSON.stringify(error, void 0, 2) + "\n");
      } else {
  ...
  decorateWithPayloads(routes, generateRoutes) {
    ...
    generateRoutes.forEach((route) => {
      const path2 = utilsEdge.isString(route) ? route : route.route;
      routeMap[path2] = {
    ...
    try {
      ...
      if (this.staticAssetsDir && renderContext.staticAssets) {
        for (const asset of renderContext.staticAssets) {
          const assetPath = path__default["default"].join(this.staticAssetsDir, ufo.decode(asset.path));
 ...
 ${html}`);
    ...
    if (this.options.generate.subFolders) {
      fileName = route === "/404" ? path__default["default"].join(path__default["default"].sep, "404.html") : path__default["default"].join(route, path__default["default"].sep, "index.html");
    } else {
  ...
  minifyHtml(html) {
    ...
    if (typeof this.options.generate.minify !== "undefined") {
      ...
      consola__default["default"].warn("generate.minify has been deprecated and will be removed in the next major version. Use build.html.minify instead!");
    }
