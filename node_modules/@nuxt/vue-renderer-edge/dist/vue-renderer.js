/*!
 * @nuxt/vue-renderer-edge v2.16.0-27260655.293dc11a (c) 2016-2021
...
class SPARenderer extends BaseRenderer {
  ...
  async render(renderContext) {
    ...
    const modern = modernMode && this.options.target === utilsEdge.TARGETS.static || utilsEdge.isModernRequest(req, modernMode);
...
class SSRRenderer extends BaseRenderer {
  ...
  addAttrs(tags, referenceTag, referenceAttr) {
    ...
    if (crossorigin) {
      tags = tags.replace(new RegExp(reference, "g"), `${reference} crossorigin="${crossorigin}"`);
    }
    return tags;
  }
  ...
  createRenderer() {
    return vueServerRenderer.createBundleRenderer(this.serverContext.resources.serverManifest, this.rendererOptions);
  }
  useSSRLog() {
  ...
  async render(renderContext) {
    ...
    if (renderContext.redirected && renderContext.target === utilsEdge.TARGETS.server) {
      return {
        html: APP,
    ...
    if (shouldInjectScripts && renderContext.staticAssetsBase) {
      ...
      if (stateScriptKb > 10) {
        const statePath = utilsEdge.urlJoin(url, "state.js");
        const stateUrl = utilsEdge.urlJoin(staticAssetsBase, statePath);
      ...
      if (!renderContext.nuxt.error && !renderContext.redirected) {
        const payloadPath = utilsEdge.urlJoin(url, "payload.js");
        const payloadUrl = utilsEdge.urlJoin(staticAssetsBase, payloadPath);
        ...
        if (this.options.generate.manifest) {
          const manifestUrl = utilsEdge.urlJoin(staticAssetsBase, "manifest.js");
    ...
    const templateParams = {
      HTML_ATTRS: meta ? meta.htmlAttrs.text(renderContext.nuxt.serverRendered) : "",
...
class ModernRenderer extends SSRRenderer {
  constructor(serverContext) {
    ...
    this.publicPath = utilsEdge.isUrl(publicPath) || ufo.isRelative(publicPath) ? publicPath : utilsEdge.urlJoin(base, publicPath);
  }
  get assetsMapping() {
  ...
  renderScripts(renderContext) {
    ...
    const safariNoModuleFixScript = `<script>${utilsEdge.safariNoModuleFix}<\/script>`;
...
class VueRenderer {
  ...
  async _ready() {
    ...
    if (this.options.modern && !this.isModernReady) {
      throw new Error(`No modern build files found in ${this.distPath}.
Use either \`nuxt build --modern\` or \`modern\` option to build modern files.`);
    } else if (!this.isReady) {
      throw new Error(`No build files found in ${this.distPath}.
Use either \`nuxt build\` or \`builder.build()\` or start nuxt in development mode.`);
  ...
  detectModernBuild() {
    ...
    const isExplicitStaticModern = options.target === utilsEdge.TARGETS.static && options.modern;
  ...
  async renderRoute(url, renderContext = {}, _retried = 0) {
    if (!this.isReady) {
      ...
      if (_retried < retryLimit && this._state !== "error") {
        await this.ready().then(() => utilsEdge.waitFor(1e3));
    ...
    if (renderContext.modern === void 0) {
      const modernMode = this.options.modern;
      renderContext.modern = modernMode === "client" || utilsEdge.isModernRequest(req, modernMode);
    }
  ...
  get resourceMap() {
    const publicPath = utilsEdge.urlJoin(this.options.app.cdnURL, this.options.app.assetsPath);
    return {
      clientManifest: {
